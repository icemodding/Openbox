...
print '<item label= "Home.%s"><action name="Execute"><execute>file-roller 
%s</execute></action></item>'  % (os.environ["USER"].capitalize(), os.environ["HOME"])

for i in xrange(len(names_and_dirs)):
    print '<item label= "%s"><action name="Execute"><execute>file-roller 
%s</execute></action></item>' % (names_and_dirs[i][0], names_and_dirs[i][1])
